...
func (c *Client) execute(command []byte) (string, bool) {
	...
	for len(tryList) > 0 {
		...
		args := &r.ExecuteArgs{Command: command}
		reply := &r.ExecuteReply{}
		if err := conn.Call(r.Execute, args, reply); err != nil {
			c.invalidateConn(id)
			continue
		}
		if reply.IsLeader {
			...
			c.leaderID = id
			c.mu.Unlock()
			return string(reply.Value), reply.Success
		}
