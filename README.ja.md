...
1.  **永続性なし:** ノードがクラッシュして再起動すると、すべてのログエントリと状態が失われます。`currentTerm`, `votedFor`, `log` はディスクに保存されません。
2.  **固定設定:** クラスタメンバーシップは静的であり、起動時に `cluster.conf` で定義されます。動的なメンバーシップ変更はサポートされていません。
3.  **スナップショットなし:** ログは無制限に増加します。ログを圧縮したりスナップショットを作成したりするメカニズムはありません。
4.  **内部専用クライアント:** 外部アプリケーションはクラスタと対話できません。`client` ロジックはサーバーバイナリ内にハードコードされています。
5.  **基本的なエラー処理:** ネットワークエラーはログに記録されますが、複雑な回復シナリオやバックオフ戦略は最小限である可能性があります。
6.  **TODO（推測）:**
